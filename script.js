...
fileInput.addEventListener('change', (e) => {
    try {
        ...
        const audioFiles = files.filter(f => f.type && (f.type.startsWith('audio/') || f.name.match(/\.(mp3|m4a|wav|ogg|flac)$/i)));
...
function renderPlaylist() {
    try {
        ...
        playlist.forEach((file, index) => {
            ...
            const cleanName = file.name ? file.name.replace(/\.[^/.]+$/, "") : "Unknown Track";
...
function initWebAudio() {
    ...
    try {
        ...
        console.log("Audio Engine Ready with fallback:", e);
    }
}
...
window.addEventListener('pointermove', (e) => {
    ...
    audio.currentTime = Math.max(0, Math.min(audio.duration, audio.currentTime + (deltaX * 0.05)));
    startX = e.clientX;
});
